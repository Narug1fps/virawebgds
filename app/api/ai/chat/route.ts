...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (/^(oi|ol[áa]|ola|olá|bom dia|boa tarde|boa noite|e aí|e ai)\b/.test(userText)) {
        try {
          ...
          return new Response("Olá! Como posso ajudar você hoje?")
        }
      }
      // Intents: counts
      ...
      if (profile) contextParts.push(`Usuário: ${profile.full_name || "(sem nome)"}${profile.clinic_name ? ` — Clínica: ${profile.clinic_name}` : ""}`)
      if (goals.length > 0) {
        contextParts.push(`Metas recentes: ${goals
          .slice(0, 5)
          .map((g: any) => `${g.title} — ${g.current_value}/${g.target_value} ${g.unit || ""}`)
          .join("; ")}`)
      }
      if (notifs.length > 0) {
        contextParts.push(`Últimas notificações: ${notifs.slice(0, 5).map((n: any) => `${n.title}: ${n.message}`).join("; ")}`)
      }

      const systemPrompt = `Você é o ViraBot, o assistente inteligente do ViraWeb, projetado para oferecer suporte abrangente e personalizado. Suas capacidades incluem:

1. Análise e sugestões: Com base nos dados do usuário, ofereça insights úteis e recomendações proativas para melhorar a gestão da clínica/consultório.

2. Suporte contextual: Use o contexto do usuário (dados, metas, notificações) para fornecer respostas relevantes e personalizadas.

3. Explicações didáticas: Explique conceitos e funcionalidades do sistema de forma clara e educativa, usando exemplos quando apropriado.

4. Geração de conteúdo: Ajude a criar descrições, resumos e textos profissionais relacionados ao contexto médico/clínico.

5. Resolução de problemas: Ofereça soluções práticas e passo a passo para dúvidas e desafios comuns.

Lembre-se: Use apenas dados reais do contexto fornecido, nunca invente informações. Mantenha um tom profissional e amigável, adequado ao ambiente de saúde.`

      const fullContext = contextParts.length > 0 ? contextParts.join("\n") : "Sem contexto adicional do usuário."
